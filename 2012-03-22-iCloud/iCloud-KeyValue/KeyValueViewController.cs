...
namespace Cloud
{
	...
	public class KeyValueViewController : UIViewController
	{
		...
		{	
			...
			saveButton.Frame = new RectangleF(10,120,90,50);
			saveButton.SetTitle("Save", UIControlState.Normal);

			reloadButton = UIButton.FromType(UIButtonType.RoundedRect);
			reloadButton.Frame = new RectangleF(110,120,90,50);
			reloadButton.SetTitle("Reload", UIControlState.Normal);
			
			clearButton = UIButton.FromType(UIButtonType.RoundedRect);
			clearButton.Frame = new RectangleF(210,120,90,50);
			clearButton.SetTitle("Clear", UIControlState.Normal);
			
			outputText = new UITextView(new RectangleF(10, 180, 300, 270));
			//outputText.BackgroundColor = UIColor.FromRGB(224,255,255);
			outputText.Editable = false;
			outputText.ScrollEnabled = true;
			
			localText = new UITextView(new RectangleF (130, 40, 150, 30));
			localText.BackgroundColor = UIColor.FromRGB(224,255,255);
			testText = new UITextView(new RectangleF (130, 80, 150, 30));
			testText.BackgroundColor = UIColor.FromRGB(224,255,255);

			localLabel = new UILabel(new RectangleF(10,  40, 100, 30));
			localLabel.Text = UIDevice.CurrentDevice.Name + ":";
			testLabel  = new UILabel(new RectangleF(10, 80, 100, 30));
			testLabel.Text = "Shared:";
			keyLabel  = new UILabel(new RectangleF(10, 10, 200, 30));
			keyLabel.Text = "KEY";
 			valueLabel = new UILabel(new RectangleF(150, 10, 200, 30));
		...
		void Save (object sender, EventArgs ea) {
			...
			var synchronized = store.Synchronize();  Console.WriteLine ("Save synchronized=" + synchronized);
		...
		void Reload (object sender, EventArgs ea) {
			var store = NSUbiquitousKeyValueStore.DefaultStore;
			var synchronized = store.Synchronize();   Console.WriteLine ("Reload synchronized=" + synchronized);
		...
		void Clear (object sender, EventArgs e) {
			...
			var synchronized = store.Synchronize();  Console.WriteLine ("Clear synchronized=" + synchronized);
			outputText.Text += String.Format ("\n--- Clear ({4}) ---\n{0}: {1}\n{2}: {3}"
